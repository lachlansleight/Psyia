...
public class AudioBarsUI : MonoBehaviour {
	...
	void Update () {
		...
		for(int i = 0; i < bars.Length; i++) {
			...
			bars[i].offsetMax = new Vector2(iT * totalWidth - (totalWidth * 0.5f) + barWidth - (barGap * 0.5f), Mathf.Clamp((audioData.fft[i] * totalHeight * 0.3f - totalHeight), -totalHeight, 0f));
		}
	}
}
